...
class basic_mapper {
 	...
 	function basic_mapper(&$parser, $createPid=0, $dbTable=null) {
 		$this->init();
		$this->parser = &$parser;
...
//		debug(array('dbSelectById' => $GLOBALS['TYPO3_DB']->SELECTquery($dbFields, $dbTable, $dbWhere)),get_class($this));
  ...
  // Include extension?
if (defined('TYPO3_MODE') && $GLOBALS['TYPO3_CONF_VARS']['TYPO3_MODE']['XCLASS']['ext/commerce/dao/class.basic_mapper.php'])	{
	include_once($GLOBALS['TYPO3_CONF_VARS']['TYPO3_MODE']['XCLASS']['ext/commerce/dao/class.basic_mapper.php']);
}
?>