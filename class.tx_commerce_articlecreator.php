...
class tx_commerce_articleCreator {
	 ...
	 * @var array
	 */
	public $flattedAttributes = array();
	 ...
	 * Constructor
	 */
	public function tx_commerce_articleCreator() {
		$this->__construct();
	}
	...
	public function existingArticles($PA) {
		...
		$articleCount = count($this->existingArticles);
		for ($i = 0; $i < $articleCount; $i++) {
			...
			if (is_array($this->attributes['ct1'])) {
				...
				foreach ($this->attributes['ct1'] as $attribute) {
					...
					$atrRes = $db->exec_SELECTquery('uid_valuelist, default_value, value_char', 'tx_commerce_articles_article_attributes_mm', 'uid_local=' . $article['uid'] . ' AND uid_foreign=' . $attribute['uid_foreign']);
					while ($attributeData = $db->sql_fetch_assoc($atrRes)) {
	 ...
	 * @return array
	 */
	public function getValues($index = 0) {
		$result = array();

		if ($index >= count($this->attributes['ct1'])) {
			return NULL;
		}
	 ...
	 * @return void
	 */
	public function getRows($data, &$resultRows, &$counter, $headRow, $extraRowData = array(), $index = 1, $row = array()) {
		if (is_array($data)) {
	 ...
	 */
	public function getHeadRow(&$colCount, $acBefore = NULL, $acAfter = NULL, $addTR = TRUE) {
		$result = '';
	...
	public function createArticles($PA) {
		if (is_array(t3lib_div::_GP('createList'))) {
			/** @var $db t3lib_db */
			$db = & $GLOBALS['TYPO3_DB'];
			$res = $db->exec_SELECTquery('uid,value', 'tx_commerce_attribute_values', 'deleted = 0');
	 ...
	 */
	public function createArticle($PA, $key) {
		/** @var $db t3lib_db */
		$db = & $GLOBALS['TYPO3_DB'];
		...
		$sorting = $db->sql_fetch_assoc($res);
		...
		foreach ($hookObjectsArr as $hookObj) {
			...
			// create the article
		$articleRes = $db->exec_INSERTquery('tx_commerce_articles', $articleData);
		$articleUid = $db->sql_insert_id($articleRes);

			// create a new price that is assigned to the new article
		$db->exec_INSERTquery('tx_commerce_article_prices', array(
				'pid' => $this->pid,
				'crdate' => time(),
				'tstamp' => time(),
		...
		$productsAttributesRes = $db->exec_SELECTquery(
			'sorting,uid_local,uid_foreign',
			'tx_commerce_products_attributes_mm',
			'uid_local = ' . intval($this->uid)
		);

		$attributesSorting = array();
		while ($productsAttributes = $db->sql_fetch_assoc($productsAttributesRes)) {
		...
		if (is_array($data)) {
			foreach ($data as $selectAttributeUid => $selectAttributeValueUid) {
				...
				$db->exec_INSERTquery('tx_commerce_articles_article_attributes_mm', $relationCreateData);
			}
		}
		...
		$resOricArticle = $db->exec_SELECTquery('*', 'tx_commerce_articles', 'uid=' . intval($articleUid) . ' and deleted = 0');
		$origArticle = $db->sql_fetch_assoc($resOricArticle);

		$resLocalisedProducts = $db->exec_SELECTquery('*', 'tx_commerce_products', 'l18n_parent=' . intval($this->uid) . ' and deleted = 0');

		if (($resLocalisedProducts) && ($db->sql_num_rows($resLocalisedProducts) > 0)) {
				// Only if there are products
			while ($localisedProducts = $db->sql_fetch_assoc($resLocalisedProducts)) {
				...
				$articleData = array(
					...
					// create the article
				$articleRes = $db->exec_INSERTquery('tx_commerce_articles', $articleData);
				$LocArticleUid = $db->sql_insert_id($articleRes);
				...
				while ($origRelation = $db->sql_fetch_assoc($res)) {
					$origRelation['uid_local'] = $LocArticleUid;

					$db->exec_INSERTquery('tx_commerce_articles_article_attributes_mm', $origRelation);
				}
	...
	public function createNewPriceCB($PA) {
		...
		$content .= $language->sL('LLL:EXT:commerce/locallang_be.php:articles.add_article_price', 1);
		$content .= '</div>';
	...
	public function createNewScalePricesCount($PA) {
		$content = '<input style="width: 77px;" class="formField1" maxlength="20" type="input" name="data[tx_commerce_articles][' .
	...
	public function deletePriceButton($PA, $fObj) {
		...
		/** @var $language language */
		$language = $GLOBALS['LANG'];
		$result .= $language->sL('LLL:EXT:commerce/locallang_be.php:articles.del_article_price', 1) . '</a>';
