	...
	function saveProductRelations($pUid, $fieldArray = NULL)	{
		...
		if ($extConf['simpleMode'] && $pUid != NULL)	{
			...
			if ($GLOBALS['TYPO3_DB']->sql_num_rows($res) == 0)	{
				...
				$pRes = $GLOBALS['TYPO3_DB']->exec_INSERTquery('tx_commerce_article_prices', array('uid_article' => $aUid));
			}
		}
...
if (defined('TYPO3_MODE') && $TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']["ext/commerce/hooks/class.tx_commerce_dmhooks.php"])	{
	include_once($TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']["ext/commerce/hooks/class.tx_commerce_dmhooks.php"]);
}
?>
