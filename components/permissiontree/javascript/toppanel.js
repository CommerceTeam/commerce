...
/**
 * @class TYPO3.Components.PermissionTree.TopPanel
 *
 * Top Panel
 *
 ...
 * @extends Ext.Panel
 */
TYPO3.Components.PermissionTree.TopPanel = Ext.extend(Ext.Panel, {
	/**
	 * Component Id
	 *
	 * @type {String}
	 */
	id: 'commerce-permissiontree-topPanel',
	 ...
	 * @type {String}
	 */
	cls: 'commerce-permissiontree-toppanel',
	 ...
	 * Filtering Tree
	 *
	 * @cfg {TYPO3.Components.PermissionTree.FilteringTree}
	 */
	filteringTree: null,
	 ...
	 * Page Tree
	 *
	 * @cfg {TYPO3.Components.PermissionTree.Tree}
	 */
	tree: null,
	 ...
	 * Application Panel
	 *
	 * @cfg {TYPO3.Components.PermissionTree.App}
	 */
	app: null,
	...
	initComponent: function() {
		this.currentlyShownPanel = new Ext.Panel({
			...
			cls: this.id + '-item commerce-permissiontree-toppanel-item',
			border: false
		});
		this.items = [this.currentlyShownPanel];

		TYPO3.Components.PermissionTree.TopPanel.superclass.initComponent.apply(this, arguments);

		//this.addDragDropNodeInsertionFeature();

		if (!TYPO3.Components.PermissionTree.Configuration.hideFilter
			|| TYPO3.Components.PermissionTree.Configuration.hideFilter === '0'
		) {
				...
				'<div class="alert alert-info">' +
					'<div class="media">' +
						...
						'<div class="media-body">' +
							TYPO3.Components.PermissionTree.LLL.activeFilterMode +
						'</div>' +
	...
	addFilterFeature: function() {
		var topPanelButton = new Ext.Button({
			...
			text: TYPO3.Components.PermissionTree.Icons.Filter,
			tooltip: TYPO3.Components.PermissionTree.LLL.buttonFilter
		});

		var textField = new Ext.form.TriggerField({
			id: this.id + '-filter',
			cls: 'form-control input-sm commerce-permissiontree-toppanel-filter',
			enableKeyEvents: true,
			triggerConfig: {
				tag: 'span',
				html: TYPO3.Components.PermissionTree.Icons.InputClear,
				cls: 'commerce-permissiontree-toppanel-filter-clear'
			},
			value: TYPO3.Components.PermissionTree.LLL.searchTermInfo,
			...
			listeners: {
				blur: {
					...
					fn:function(textField) {
						if (textField.getValue() === '') {
							textField.setValue(TYPO3.Components.PermissionTree.LLL.searchTermInfo);
				...
				focus: {
					scope: this,
					fn: function(textField) {
						if (textField.getValue() === TYPO3.Components.PermissionTree.LLL.searchTermInfo) {
							textField.setValue('');
	...
	createNewNodeToolbar: function() {
		this.dragZone = new Ext.dd.DragZone(this.getEl(), {
			...
			onInitDrag: function() {
				...
				this.proxyElement.innerHTML = '<div class="x-dd-drag-ghost-permissiontree">' +
					'<span class="x-dd-drag-ghost-permissiontree-icon">' + clickedButton.initialConfig.html + '</span>' +
					'<span class="x-dd-drag-ghost-permissiontree-text">'  + clickedButton.title + '</span>' +
				'</div>';
	...
	addDragDropNodeInsertionFeature: function() {
		...
		this.dataProvider.getNodeTypes(function(response) {
			...
			if (amountOfNodeTypes > 0) {
				...
				for (var i = 0; i < amountOfNodeTypes; ++i) {
					...
					response[i].cls = 'commerce-permissiontree-toppanel-drag-node';
		...
		var topPanelButton = new Ext.Button({
			...
			cls: 'btn btn-default btn-sm',
			text: TYPO3.Components.PermissionTree.Icons.NewNode,
			tooltip: TYPO3.Components.PermissionTree.LLL.buttonNewNode,
			hidden: true
		});
	...
	addRefreshTreeFeature: function() {
		var topPanelButton = new Ext.Button({
			...
			cls: 'btn btn-default btn-sm',
			text: TYPO3.Components.PermissionTree.Icons.Refresh,
			tooltip: TYPO3.Components.PermissionTree.LLL.buttonRefresh,
...
// XTYPE Registration
Ext.reg('TYPO3.Components.PermissionTree.TopPanel', TYPO3.Components.PermissionTree.TopPanel);
