...
/**
 * @class TYPO3.Components.PermissionTree.Actions
 *
 * Actions dedicated for the page tree
 *
 * @namespace TYPO3.Components.PermissionTree
 */
TYPO3.Components.PermissionTree.Actions = {
	 ...
	 *
	 * @param {TYPO3.Components.PermissionTree.Tree} tree
	 * @return {void}
	 */
	...
	confirmDelete: function(node, tree, callback, recursiveDelete) {
		...
		var title = TYPO3.Components.PermissionTree.LLL.deleteDialogTitle,
			message = TYPO3.Components.PermissionTree.LLL.deleteDialogMessage;
		if (recursiveDelete) {
			message = TYPO3.Components.PermissionTree.LLL.recursiveDeleteDialogMessage;
		}
		...
		Ext.Msg.show({
			...
			fn: function (answer) {
				if (answer === 'yes') {
					TYPO3.Components.PermissionTree.Actions.deleteNode(node, tree, callback);
					return true;
				}
				return false;
			},
	...
	deleteNode: function(node, tree, callback) {
		TYPO3.Components.PermissionTree.Commands.deleteNode(
	...
	removeNode: function(node, tree) {
		if (TYPO3.Components.PermissionTree.Configuration.displayDeleteConfirmation) {
	...
	restoreNodeToFirstChildOfDestination: function(node, tree, destination) {
		TYPO3.Components.PermissionTree.Commands.restoreNode(
			node.attributes.nodeData,
	...
	viewPage: function(node) {
		...
		TYPO3.Components.PermissionTree.Commands.getViewLink(
			node.attributes.nodeData,
			function(result) {
	...
	mountAsTreeRoot: function(node, tree) {
		TYPO3.Components.PermissionTree.Commands.setTemporaryMountPoint(
			node.attributes.nodeData,
			function(response) {
				if (TYPO3.Components.PermissionTree.Configuration.temporaryMountPoint) {
					TYPO3.Backend.NavigationContainer.PermissionTree.removeIndicator(
						TYPO3.Backend.NavigationContainer.PermissionTree.temporaryMountPointInfoIndicator
					);
				}

				TYPO3.Components.PermissionTree.Configuration.temporaryMountPoint = response;
				TYPO3.Backend.NavigationContainer.PermissionTree.addTemporaryMountPointIndicator();

				var selectedNode = TYPO3.Backend.NavigationContainer.PermissionTree.getSelected();
				tree.stateId = 'PermissionTree' + TYPO3.Components.PermissionTree.Configuration.temporaryMountPoint;
				tree.refreshTree(function() {
					var nodeIsSelected = false;
					if (selectedNode) {
						nodeIsSelected = TYPO3.Backend.NavigationContainer.PermissionTree.select(
							selectedNode.attributes.nodeData.id
						);
					}

					var node = (nodeIsSelected ? TYPO3.Backend.NavigationContainer.PermissionTree.getSelected() : null);
					if (node) {
	...
	moveNodeAfterDestination: function(node, tree) {
		TYPO3.Components.PermissionTree.Commands.moveNodeAfterDestination(
	...
	moveNodeToFirstChildOfDestination: function(node, tree) {
		TYPO3.Components.PermissionTree.Commands.moveNodeToFirstChildOfDestination(
			tree.t3ContextNode.attributes.nodeData,
			node.attributes.nodeData.id,
			function(response) {
				if (this.evaluateResponse(response) && tree.t3ContextNode) {
	...
	insertNodeAfterDestination: function(node, tree) {
		TYPO3.Components.PermissionTree.Commands.insertNodeAfterDestination(
			tree.t3ContextNode.attributes.nodeData,
	...
	insertNodeToFirstChildOfDestination: function(node, tree) {
		TYPO3.Components.PermissionTree.Commands.insertNodeToFirstChildOfDestination(
			tree.t3ContextNode.attributes.nodeData,
			tree.t3ContextInfo.serverNodeType,
			function(response) {
				if (this.evaluateResponse(response)) {
	...
	copyNodeAfterDestination: function(node, tree) {
		TYPO3.Components.PermissionTree.Commands.copyNodeAfterDestination(
			tree.t3ContextNode.attributes.nodeData,
			node.previousSibling.attributes.nodeData.id,
			function(response) {
				if (this.evaluateResponse(response)) {
	...
	copyNodeToFirstChildOfDestination: function(node, tree) {
		TYPO3.Components.PermissionTree.Commands.copyNodeToFirstChildOfDestination(
			tree.t3ContextNode.attributes.nodeData,
	...
	enablePage: function(node) {
		TYPO3.Components.PermissionTree.Commands.visiblyNode(
			node.attributes.nodeData,
			function(response) {
				if (this.evaluateResponse(response)) {
					this.updateNode(node, node.isExpanded(), response);
	...
	disablePage: function(node) {
		TYPO3.Components.PermissionTree.Commands.disableNode(
			node.attributes.nodeData,
			function(response) {
				if (this.evaluateResponse(response)) {
					this.updateNode(node, node.isExpanded(), response);
	 ...
	 * @param {TYPO3.Components.PermissionTree.TreeEditor} treeEditor
	 * @return {void}
	 */
	saveTitle: function(newText, oldText, treeEditor) {
		...
		TYPO3.Components.PermissionTree.Commands.updateLabel(
