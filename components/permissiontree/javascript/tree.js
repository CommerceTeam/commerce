...
/**
 * @class TYPO3.Components.PermissionTree.Tree
 *
 * Generic Tree Panel
 *
 ...
 */
TYPO3.Components.PermissionTree.Tree = Ext.extend(Ext.tree.TreePanel, {
	/**
	 * Border
	 *
	 * @type {Boolean}
	 ...
	 *
	 * @type {TYPO3.Components.PermissionTree.TreeEditor}
	 */
	treeEditor: null,
	...
	initComponent: function() {
		if (!this.uiProvider) {
			this.uiProvider = TYPO3.Components.PermissionTree.PageTreeNodeUI;
		}
		...
		TYPO3.Components.PermissionTree.Tree.superclass.initComponent.apply(this, arguments);
	},
	...
	enableContextMenu: function() {
		this.contextMenu = new TYPO3.Components.PermissionTree.ContextMenu();
	...
	enableInlineEditor: function() {
		this.treeEditor = new TYPO3.Components.PermissionTree.TreeEditor(this);
	},
		...
		(new Ext.KeyMap(document, {
			...
			fn: function() {
				if (!this.controlKeyPressed && this.dragZone.dragging && this.copyHint) {
					...
					this.dragZone.proxy.el.toggleClass('commerce-permissiontree-copy');
				}
	 ...
	 *
	 * @param {TYPO3.Components.PermissionTree.Tree} treePanel
	 * @param {Ext.tree.TreeNode} node
	 * @return {void}
	 */
	initDd: function(treePanel, node) {
		...
		if (this.deletionDropZoneId &&
			(!nodeHasChildNodes ||
			(nodeHasChildNodes && TYPO3.Components.PermissionTree.Configuration.canDeleteRecursivly)
		)) {
	...
	initDDProxyElement: function() {
		...
		this.copyHint.dom.appendChild(document.createTextNode(TYPO3.Components.PermissionTree.LLL.copyHint));
	...
	beforeDropNode: function(dragElement) {
		if (dragElement.data && dragElement.data.item && dragElement.data.item.shouldCreateNewNode) {
			...
			dragElement.dropNode = new Ext.tree.TreeNode({
				text: TYPO3.Components.PermissionTree.LLL.fakeNodeHint,
				leaf: true,
				isInsertedNode: true
			});
...
// XTYPE Registration
Ext.reg('TYPO3.Components.PermissionTree.Tree', TYPO3.Components.PermissionTree.Tree);
