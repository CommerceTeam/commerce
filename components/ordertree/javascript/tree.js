...
/**
 * @class TYPO3.Components.OrderTree.Tree
 *
 * Generic Tree Panel
 *
 ...
 */
TYPO3.Components.OrderTree.Tree = Ext.extend(Ext.tree.TreePanel, {
	/**
	 * Border
	 *
	 * @type {Boolean}
	 ...
	 *
	 * @type {TYPO3.Components.OrderTree.TreeEditor}
	 */
	treeEditor: null,
	...
	initComponent: function() {
		if (!this.uiProvider) {
			this.uiProvider = TYPO3.Components.OrderTree.PageTreeNodeUI;
		}
		...
		TYPO3.Components.OrderTree.Tree.superclass.initComponent.apply(this, arguments);
	},
	...
	enableContextMenu: function() {
		this.contextMenu = new TYPO3.Components.OrderTree.ContextMenu();
	...
	enableInlineEditor: function() {
		this.treeEditor = new TYPO3.Components.OrderTree.TreeEditor(this);
	},
		...
		(new Ext.KeyMap(document, {
			...
			fn: function() {
				if (!this.controlKeyPressed && this.dragZone.dragging && this.copyHint) {
					...
					this.dragZone.proxy.el.toggleClass('commerce-ordertree-copy');
				}
	 ...
	 *
	 * @param {TYPO3.Components.OrderTree.Tree} treePanel
	 * @param {Ext.tree.TreeNode} node
	 * @return {void}
	 */
	initDd: function(treePanel, node) {
		...
		if (this.deletionDropZoneId &&
			(!nodeHasChildNodes ||
			(nodeHasChildNodes && TYPO3.Components.OrderTree.Configuration.canDeleteRecursivly)
		)) {
	...
	initDDProxyElement: function() {
		...
		this.copyHint.dom.appendChild(document.createTextNode(TYPO3.Components.OrderTree.LLL.copyHint));
	...
	beforeDropNode: function(dragElement) {
		if (dragElement.data && dragElement.data.item && dragElement.data.item.shouldCreateNewNode) {
			...
			dragElement.dropNode = new Ext.tree.TreeNode({
				text: TYPO3.Components.OrderTree.LLL.fakeNodeHint,
				leaf: true,
				isInsertedNode: true
			});
...
// XTYPE Registration
Ext.reg('TYPO3.Components.OrderTree.Tree', TYPO3.Components.OrderTree.Tree);
