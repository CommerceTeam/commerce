...
/**
 * @class TYPO3.Components.OrderTree.Actions
 *
 * Actions dedicated for the page tree
 *
 * @namespace TYPO3.Components.OrderTree
 */
TYPO3.Components.OrderTree.Actions = {
	 ...
	 *
	 * @param {TYPO3.Components.OrderTree.Tree} tree
	 * @return {void}
	 */
	...
	confirmDelete: function(node, tree, callback, recursiveDelete) {
		...
		var title = TYPO3.Components.OrderTree.LLL.deleteDialogTitle,
			message = TYPO3.Components.OrderTree.LLL.deleteDialogMessage;
		if (recursiveDelete) {
			message = TYPO3.Components.OrderTree.LLL.recursiveDeleteDialogMessage;
		}
		...
		Ext.Msg.show({
			...
			fn: function (answer) {
				if (answer === 'yes') {
					TYPO3.Components.OrderTree.Actions.deleteNode(node, tree, callback);
					return true;
				}
				return false;
			},
	...
	deleteNode: function(node, tree, callback) {
		TYPO3.Components.OrderTree.Commands.deleteNode(
	...
	removeNode: function(node, tree) {
		if (TYPO3.Components.OrderTree.Configuration.displayDeleteConfirmation) {
	...
	restoreNodeToFirstChildOfDestination: function(node, tree, destination) {
		TYPO3.Components.OrderTree.Commands.restoreNode(
			node.attributes.nodeData,
	...
	viewPage: function(node) {
		...
		TYPO3.Components.OrderTree.Commands.getViewLink(
			node.attributes.nodeData,
			function(result) {
	...
	mountAsTreeRoot: function(node, tree) {
		TYPO3.Components.OrderTree.Commands.setTemporaryMountPoint(
			node.attributes.nodeData,
			function(response) {
				if (TYPO3.Components.OrderTree.Configuration.temporaryMountPoint) {
					TYPO3.Backend.NavigationContainer.OrderTree.removeIndicator(
						TYPO3.Backend.NavigationContainer.OrderTree.temporaryMountPointInfoIndicator
					);
				}
				...
				tree.refreshTree(function() {
					...
					var node = (nodeIsSelected ? TYPO3.Backend.NavigationContainer.OrderTree.getSelected() : null);
					if (node) {
	...
	moveNodeAfterDestination: function(node, tree) {
		TYPO3.Components.OrderTree.Commands.moveNodeAfterDestination(
	...
	moveNodeToFirstChildOfDestination: function(node, tree) {
		TYPO3.Components.OrderTree.Commands.moveNodeToFirstChildOfDestination(
			tree.t3ContextNode.attributes.nodeData,
			node.attributes.nodeData.id,
			function(response) {
				if (this.evaluateResponse(response) && tree.t3ContextNode) {
	...
	insertNodeAfterDestination: function(node, tree) {
		TYPO3.Components.OrderTree.Commands.insertNodeAfterDestination(
			tree.t3ContextNode.attributes.nodeData,
	...
	insertNodeToFirstChildOfDestination: function(node, tree) {
		TYPO3.Components.OrderTree.Commands.insertNodeToFirstChildOfDestination(
			tree.t3ContextNode.attributes.nodeData,
			tree.t3ContextInfo.serverNodeType,
			function(response) {
				if (this.evaluateResponse(response)) {
	...
	copyNodeAfterDestination: function(node, tree) {
		TYPO3.Components.OrderTree.Commands.copyNodeAfterDestination(
			tree.t3ContextNode.attributes.nodeData,
			node.previousSibling.attributes.nodeData.id,
			function(response) {
				if (this.evaluateResponse(response)) {
	...
	copyNodeToFirstChildOfDestination: function(node, tree) {
		TYPO3.Components.OrderTree.Commands.copyNodeToFirstChildOfDestination(
			tree.t3ContextNode.attributes.nodeData,
	...
	enablePage: function(node) {
		TYPO3.Components.OrderTree.Commands.visiblyNode(
			node.attributes.nodeData,
			function(response) {
				if (this.evaluateResponse(response)) {
					this.updateNode(node, node.isExpanded(), response);
	...
	disablePage: function(node) {
		TYPO3.Components.OrderTree.Commands.disableNode(
			node.attributes.nodeData,
			function(response) {
				if (this.evaluateResponse(response)) {
					this.updateNode(node, node.isExpanded(), response);
	 ...
	 * @param {TYPO3.Components.OrderTree.TreeEditor} treeEditor
	 * @return {void}
	 */
	saveTitle: function(newText, oldText, treeEditor) {
		...
		TYPO3.Components.OrderTree.Commands.updateLabel(
