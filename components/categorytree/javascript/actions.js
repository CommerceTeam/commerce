...
/**
 * @class TYPO3.Components.CategoryTree.Actions
 *
 * Actions dedicated for the page tree
 *
 * @namespace TYPO3.Components.CategoryTree
 */
TYPO3.Components.CategoryTree.Actions = {
	 ...
	 *
	 * @param {TYPO3.Components.CategoryTree.Tree} tree
	 * @return {void}
	 */
	...
	confirmDelete: function(node, tree, callback, recursiveDelete) {
		...
		var title = TYPO3.Components.CategoryTree.LLL.deleteDialogTitle,
			message = TYPO3.Components.CategoryTree.LLL.deleteDialogMessage;
		if (recursiveDelete) {
			message = TYPO3.Components.CategoryTree.LLL.recursiveDeleteDialogMessage;
		}
		...
		Ext.Msg.show({
			...
			fn: function (answer) {
				if (answer === 'yes') {
					TYPO3.Components.CategoryTree.Actions.deleteNode(node, tree, callback);
					return true;
				}
				return false;
			},
	...
	deleteNode: function(node, tree, callback) {
		TYPO3.Components.CategoryTree.Commands.deleteNode(
	...
	removeNode: function(node, tree) {
		if (TYPO3.Components.CategoryTree.Configuration.displayDeleteConfirmation) {
	...
	restoreNodeToFirstChildOfDestination: function(node, tree, destination) {
		TYPO3.Components.CategoryTree.Commands.restoreNode(
			node.attributes.nodeData,
	...
	viewPage: function(node) {
		...
		TYPO3.Components.CategoryTree.Commands.getViewLink(
			node.attributes.nodeData,
			function(result) {
	...
	mountAsTreeRoot: function(node, tree) {
		TYPO3.Components.CategoryTree.Commands.setTemporaryMountPoint(
			node.attributes.nodeData,
			function(response) {
				if (TYPO3.Components.CategoryTree.Configuration.temporaryMountPoint) {
					TYPO3.Backend.NavigationContainer.CategoryTree.removeIndicator(
						TYPO3.Backend.NavigationContainer.CategoryTree.temporaryMountPointInfoIndicator
					);
				}
				...
				tree.refreshTree(function() {
					...
					var node = (nodeIsSelected ? TYPO3.Backend.NavigationContainer.CategoryTree.getSelected() : null);
					if (node) {
	...
	moveNodeAfterDestination: function(node, tree) {
		TYPO3.Components.CategoryTree.Commands.moveNodeAfterDestination(
	...
	moveNodeToFirstChildOfDestination: function(node, tree) {
		TYPO3.Components.CategoryTree.Commands.moveNodeToFirstChildOfDestination(
			tree.t3ContextNode.attributes.nodeData,
			node.attributes.nodeData.id,
			function(response) {
				if (this.evaluateResponse(response) && tree.t3ContextNode) {
	...
	insertNodeAfterDestination: function(node, tree) {
		TYPO3.Components.CategoryTree.Commands.insertNodeAfterDestination(
			tree.t3ContextNode.attributes.nodeData,
	...
	insertNodeToFirstChildOfDestination: function(node, tree) {
		TYPO3.Components.CategoryTree.Commands.insertNodeToFirstChildOfDestination(
			tree.t3ContextNode.attributes.nodeData,
			tree.t3ContextInfo.serverNodeType,
			function(response) {
				if (this.evaluateResponse(response)) {
	...
	copyNodeAfterDestination: function(node, tree) {
		TYPO3.Components.CategoryTree.Commands.copyNodeAfterDestination(
			tree.t3ContextNode.attributes.nodeData,
			node.previousSibling.attributes.nodeData.id,
			function(response) {
				if (this.evaluateResponse(response)) {
	...
	copyNodeToFirstChildOfDestination: function(node, tree) {
		TYPO3.Components.CategoryTree.Commands.copyNodeToFirstChildOfDestination(
			tree.t3ContextNode.attributes.nodeData,
	 ...
	 * @param {TYPO3.Components.CategoryTree.TreeEditor} treeEditor
	 * @return {void}
	 */
	saveTitle: function(newText, oldText, treeEditor) {
		...
		TYPO3.Components.CategoryTree.Commands.updateLabel(
