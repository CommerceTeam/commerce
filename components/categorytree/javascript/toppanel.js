...
/**
 * @class TYPO3.Components.CategoryTree.TopPanel
 *
 * Top Panel
 *
 ...
 * @extends Ext.Panel
 */
TYPO3.Components.CategoryTree.TopPanel = Ext.extend(Ext.Panel, {
	/**
	 * Component Id
	 *
	 * @type {String}
	 */
	id: 'commerce-categorytree-topPanel',
	 ...
	 * @type {String}
	 */
	cls: 'commerce-categorytree-toppanel',
	 ...
	 * Filtering Tree
	 *
	 * @cfg {TYPO3.Components.CategoryTree.FilteringTree}
	 */
	filteringTree: null,
	 ...
	 * Application Panel
	 *
	 * @cfg {TYPO3.Components.CategoryTree.App}
	 */
	app: null,
	...
	initComponent: function() {
		this.currentlyShownPanel = new Ext.Panel({
			...
			cls: this.id + '-item commerce-categorytree-toppanel-item',
			border: false
		});
		this.items = [this.currentlyShownPanel];

		TYPO3.Components.CategoryTree.TopPanel.superclass.initComponent.apply(this, arguments);
				...
				'<div class="alert alert-info">' +
					'<div class="media">' +
						...
						'<div class="media-body">' +
							TYPO3.Components.CategoryTree.LLL.activeFilterMode +
						'</div>' +
	...
	addFilterFeature: function() {
		var topPanelButton = new Ext.Button({
			...
			text: TYPO3.Components.CategoryTree.Icons.Filter,
			tooltip: TYPO3.Components.CategoryTree.LLL.buttonFilter
		});

		var textField = new Ext.form.TriggerField({
			id: this.id + '-filter',
			cls: 'form-control input-sm commerce-categorytree-toppanel-filter',
			enableKeyEvents: true,
			triggerConfig: {
				tag: 'span',
				html: TYPO3.Components.CategoryTree.Icons.InputClear,
				cls: 'commerce-categorytree-toppanel-filter-clear'
			},
			value: TYPO3.Components.CategoryTree.LLL.searchTermInfo,
			...
			listeners: {
				blur: {
					...
					fn:function(textField) {
						if (textField.getValue() === '') {
							textField.setValue(TYPO3.Components.CategoryTree.LLL.searchTermInfo);
				...
				focus: {
					scope: this,
					fn: function(textField) {
						if (textField.getValue() === TYPO3.Components.CategoryTree.LLL.searchTermInfo) {
							textField.setValue('');
	...
	addDragDropNodeInsertionFeature: function() {
		...
		this.dataProvider.getNodeTypes(function(response) {
			...
			if (amountOfNodeTypes > 0) {
				...
				for (var i = 0; i < amountOfNodeTypes; ++i) {
					...
					response[i].cls = 'commerce-categorytree-toppanel-drag-node';
		...
		var topPanelButton = new Ext.Button({
			...
			cls: 'btn btn-default btn-sm',
			text: TYPO3.Components.CategoryTree.Icons.NewNode,
			tooltip: TYPO3.Components.CategoryTree.LLL.buttonNewNode,
			hidden: true
		});
	...
	addRefreshTreeFeature: function() {
		var topPanelButton = new Ext.Button({
			...
			cls: 'btn btn-default btn-sm',
			text: TYPO3.Components.CategoryTree.Icons.Refresh,
			tooltip: TYPO3.Components.CategoryTree.LLL.buttonRefresh,
...
// XTYPE Registration
Ext.reg('TYPO3.Components.CategoryTree.TopPanel', TYPO3.Components.CategoryTree.TopPanel);
