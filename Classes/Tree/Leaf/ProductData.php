...
class Tx_Commerce_Tree_Leaf_ProductData extends Tx_Commerce_Tree_Leaf_SlaveData {
	...
	public function getRecordsDbList($uid, $depth = 2) {
		...
		if (!is_numeric($uid) || !is_numeric($depth)) {
			if (TYPO3_DLOG) {
				\TYPO3\CMS\Core\Utility\GeneralUtility::devLog(
					'getRecordsDbList (productdata) gets passed invalid parameters.', COMMERCE_EXTKEY, 3
				);
			}
			return NULL;
		}
		...
		if (!$backendUser->check('tables_select', $this->table)) {
			if (TYPO3_DLOG) {
				\TYPO3\CMS\Core\Utility\GeneralUtility::devLog(
					'getRecordsDbList (productdata): Usergroup is not allowed to view records.', COMMERCE_EXTKEY, 2
				);
			}
			return NULL;
		}
