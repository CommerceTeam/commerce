...
class AddressesController extends BaseController
{
    ...
    public function getListing(
        ...
        foreach ($this->addresses as $address) {
            ...
            if ((int) $addressTypeCounter[$address['tx_commerce_address_type_id']]
            ...
            $linkMarkerArray['###LINK_EDIT###'] = explode(
                '|',
            ...
            if ($editAddressId == $address['uid']
                || (empty($editAddressId) && $address['tx_commerce_is_main_address'])
            ) {
        ...
        if ($addressType == 0) {
            ...
            $linkMarkerArray['###LINK_NEW###'] = explode(
                '|',
                $this->pi_linkTP_keepPIvars(
                    '|',
                    array_merge(
    ...
    protected function getAddressForm(
        ...
        foreach ($hooks as $hookObj) {
            if (method_exists($hookObj, 'processAddressFormMarker')) {
                $hookObj->processAddressFormMarker(
                    $baseMarkerArray,
                    $action,
                    $addressUid,
                    $addressData,
    ...
    protected function deleteAddressQuestion()
    {
        ...
        $baseMarkerArray['YES'] = $this->cObj->stdWrap(
            $this->pi_getLL('label_submit_yes'),
            $this->conf['yesLinkWrap.']
        );
