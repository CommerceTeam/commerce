...
class tx_commerce_db_category extends tx_commerce_db_alib {
	 ...
	 */
	protected $database_table = 'tx_commerce_categories';
	 ...
	 */
	protected $database_attribute_rel_table = 'tx_commerce_categories_attributes_mm';
	...
	public function getPermissionsRecord($uid) {
 	...
 	public function get_parent_categories($uid) {
 		...
 		if (is_object($GLOBALS['TSFE']->sys_page)) {
 			...
 			$add_where = '';
                ...
                $result = $GLOBALS['TYPO3_DB']->exec_SELECT_mm_query('uid_foreign',
                        $this->database_table,
 	...
 	public function get_l18n_categories($uid) {
 		...
 		$this->uid = $uid;
 		$res = $GLOBALS['TYPO3_DB']->exec_SELECTquery('t1.title, t1.uid, t2.flag, t2.uid as sys_language', 
	...
	public function getChildCategories($uid) {
 	...
 	public function get_child_categories($uid) 	{
 		 ...
 		 * As TYPO3 is´nt curretly able to sort by MM tables (or we haven't found a way to use it)
		...
		if (method_exists($hookObj, 'categoryOrder')) {
 		...
 		if (is_object($GLOBALS['TSFE']->sys_page)) {
 			...
 			$add_where = '';
                ...
                $result = $GLOBALS['TYPO3_DB']->exec_SELECT_mm_query('uid_local',
                        $this->database_table,
                        $this->mm_database_table,
                        $this->database_table,
